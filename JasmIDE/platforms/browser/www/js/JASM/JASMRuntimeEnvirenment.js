	...
	this.removeGarbage = function(data) {
		...
		for(var index in lines) {
			if(lines[index] != "") {
				...
				if(lineWithoutGarbage != undefined && lineWithoutGarbage != "") result.push(lineWithoutGarbage);
			}
		}
		return result;
	};
...
function Devices() {
	this.devices = [
		new DeviceDecSendOutput(), // 0
		new DeviceSignedDecSendOutput(), // 1
		new DeviceCharSendOutput(), // 2
		new DeviceDecOutput(), // 3
		new DeviceCharOutput(), // 4
		new DeviceDecInput(), // 5
		new DeviceSignedDecInput(), // 6
		new DeviceStringInput(), // 7
		new DeviceCharInput() // 8
	];
	this.setting = {
		buffers : {
			input : "",
...
function DeviceDecSendOutput() {
	this.in = function(memory, information, setting) {
		console.log(information.parseDec().toString());
...
function DeviceDecOutput() {
	this.in = function(memory, information, setting) {
		...
		{
			setting.streams.outputElement.innerHTML = setting.streams.outputElement.innerHTML + parseInt(setting.buffers.output, 10).toString();
...
function DeviceSignedDecInput() {
	...
	this.out = function(memory, information, setting) {
		mem = new NumberMemory(parseInt(setting.buffers.input, 10));
		console.log(information);
		memory.setMemory(mem, information.parseDec());
		setting.buffers.input = "";
		return {
...
function DeviceCharInput() {
	...
	this.out = function(memory, information, setting) {
		mem = new NumberMemory(parseInt(setting.buffers.input.charCodeAt(setting.buffers.input.length-1), 10));
		console.log(information);
		memory.setMemory(mem, information.parseDec());
		setting.buffers.input = setting.buffers.input.substr(0, setting.buffers.input.length-1);
		return {
			Memory: memory,
			Setting: setting
		};
